...
GPUCFLAGS = -c -gencode=arch=compute_30,code=sm_30 --expt-relaxed-constexpr
LFLAGS    = -pthread -shared -fPIC
GPULFLAGS = -x cu -Xcompiler -fPIC
CGPUFLAGS = -L$(CUDA_HOME)/lib -L$(CUDA_HOME)/lib64 -L ~/anaconda3/lib/python3.6/site-packages/tensorflow -ltensorflow_framework -lcudart 
